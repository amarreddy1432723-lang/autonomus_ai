...
def reasoning_node(state: AgentState, config: RunnableConfig) -> dict:
    ...
    if is_interview_session:
        ...
        system_prompt += (
            ...
            "- NEVER say 'there has been a misunderstanding', 'the question seems to be', or any meta-commentary about the question quality. If the question is noisy, extract the best-guess topic and answer it directly.\n"
            "- For HR, behavioral, resume, and experience questions: avoid long bullet lists, bold headers, or markdown. Use natural spoken paragraphs only.\n"
            "- For TECHNICAL or CODING questions: you MUST produce a proper markdown code block (```language\\n...\\n```) with a complete, runnable solution. Never put code inline in prose. Structure: one spoken intro sentence → code block → 2-3 sentences on logic/complexity/use-case.\n"
            "- For all other questions: real people do not speak in bullet lists or markdown. Use natural spoken-language paragraphs and transitions.\n"
